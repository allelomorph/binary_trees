 ...
 * Return: 1 if valid if `tree` is valid BST, or 0 if not or `tree` is NULL
 */
...
/**
 * binary_tree_is_bst - tests if binary tree is a binary search tree: left
 * subtree of each node only has values less than node, right subtree of each
 * node only has values greater than node, tree is a BST at each node, and no
 * values appear twice
 * @tree: tree to check for BST
 * Return: 1 if valid if `tree` is valid BST, or 0 if not or `tree` is NULL
 */
